...
class ViewModel {
    ...
    func load() async {
        ...
        if (try! await documentStore.listDocumentIds().isEmpty) {
    ...
    var waitForConnectionScope: CoroutineScope? = nil
    
    func cancelPresentment() async {
        print("cancelPresentment()")
        waitForConnectionScope?.cancel(cause: nil)
        waitForConnectionScope = nil
        qrCode = nil
        if path.count > 0 {
            path.removeLast()
        }
    ...
    // Starts advertising MdocTransports, generates a QR code and navigates to Destination.QrScreen.
    //
    // Also sets up a task for waiting for inbound connections.
    //
    //
    func startPresentment() async {
        print("startPresentment(): path: \(path)")
        ...
        let deviceEngagementBuilder = DeviceEngagement.Builder(eDeviceKey: eDeviceKey.publicKey, version: nil)
        connectionMethods.forEach { connectionMethod in
            deviceEngagementBuilder.addConnectionMethod(connectionMethod: connectionMethod)
        }
        let deviceEngagement = deviceEngagementBuilder.build()
        let encodedDeviceEngagement = Cbor.shared.encode(item: deviceEngagement.toDataItem())
        ...
        waitForConnectionScope = CoroutineScope(context: Dispatchers.shared.Default)
        let transport = try! await ConnectionHelperKt.waitForConnection(
            advertisedTransports,
            eSenderKey: eDeviceKey.publicKey
        )
        print("We've got a transport \(transport)")
        path.removeLast()
        path.append(Destination.transferView)
        
        var sessionEncryption: SessionEncryption? = nil
        var sessionTranscript: DataItem? = nil
        while true {
            let encryptedMessage = try! await transport.waitForMessage()
            if (sessionEncryption == nil) {
                let eReaderKey = SessionEncryption.companion.getEReaderKey(sessionEstablishmentMessage: encryptedMessage)
                sessionTranscript = CborArray(items: [
                    Tagged(tagNumber: Tagged.companion.ENCODED_CBOR, taggedItem: Bstr(value: encodedDeviceEngagement)),
                    Tagged(tagNumber: Tagged.companion.ENCODED_CBOR, taggedItem: Bstr(value: eReaderKey.encodedCoseKey)),
                    Simple.companion.NULL
                ], indefiniteLength: false)
                sessionEncryption = SessionEncryption(
                    role: MdocRole.mdoc,
                    eSelfKey: eDeviceKey,
                    remotePublicKey: eReaderKey.publicKey,
                    encodedSessionTranscript: Cbor.shared.encode(item: sessionTranscript!)
                )
            }
            let decryptedMessage = try! await sessionEncryption?.decryptMessage(messageData: encryptedMessage)
            let message = decryptedMessage!.first as KotlinByteArray?
            let status = (decryptedMessage!.second!).intValue
            if status == Constants.shared.SESSION_DATA_STATUS_SESSION_TERMINATION {
                print("Received session termination from reader")
                break
            }
            if message == nil {
                print("No message from reader")
                break
            }
            Logger.shared.dCbor(tag: "TAG", message: "We got a message", encodedCbor: message!)
        }
