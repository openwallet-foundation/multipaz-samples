...
val projectVersionName: String by extra {
    lazy {
        if (projectVersionNext.isEmpty()) {
            ...
            // If the tag doesn't exist, numCommitsSinceTag will be empty, so fall back to total commit count
            val commitCount = if (numCommitsSinceTag.isEmpty()) {
                runCommand(listOf("git", "rev-list", "HEAD", "--count"))
            } else {
                numCommitsSinceTag
            }
            projectVersionNext + "-pre.${commitCount}.${commitHash}"
        }
    }.value
}
